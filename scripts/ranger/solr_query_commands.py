...
def main():
  ...
  system_requests = SystemRequests('queries')
...
def execute(system_requests, machine_config, no_of_execution):
  ...
  document_number = int(machine_config.get('ambari-infra', 'document_number', '10'))
  ip_address = machine_config.get('host', 'ip_address', None)
  cluster = machine_config.get('cluster', 'cluster_name', None)

  sleep_time = int(machine_config.get('ambari-infra', 'sleep_time', '100'))
  ...
  for i in range(0, no_of_execution):
    ...
    curl = '{0} -X GET'.format(curl_prefix)

    url = '{0}/solr/{1}/select?q=*:*&rows=25&wt=json'.format(solr_server_address, solr_ranger_collection)

    curl_command = '{0} \'{1}\''.format(curl, url)
